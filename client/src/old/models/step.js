...
class Step {
  ...
  constructor(options) {
     ...
     * @public
     */
    this.node = Store.create("Node", options);
  ...
  get openedNodes() {
    if (!this._openedNodes) {
      this._openedNodes = this.nodes
        .filter((node) => !this.closedNodes.includes(node))
        .filter((node) => !this.frontierNodes.includes(node));
    }
