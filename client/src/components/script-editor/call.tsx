...
type ReturnTypeOf<T extends Key> =
  TemplateMap[T] extends FunctionTemplate<[...unknown[]], infer R> ? R : never;

type ParamsOf<T extends Key> = TemplateMap[T] extends FunctionTemplate<infer R, never> ? R : [];

const fn = memo(
  (script: string, method: string) =>
    new Function("params", `${script}; return ${method}.apply(null, params);`) as (
      params: any[],
    ) => any,
);
