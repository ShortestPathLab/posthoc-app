...
export function useLoadFile() {
  ...
  const isTraceLoaded = useMemo(() => !!(specimen.interlang && specimen.eventList), [specimen.interlang, specimen.eventList]);
  ...
  const removeFile = useCallback((type: FileTypes) => {
    switch(type) {
      ...
      case "trace": {
        setSpecimen({...specimen, interlang: undefined, eventList: undefined}); 
        break;
      }
    }
  }, [])
