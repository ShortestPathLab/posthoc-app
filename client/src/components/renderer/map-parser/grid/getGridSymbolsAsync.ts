...
export const getGridSymbolsQuery = (params: GetGridSymbolsParameters) =>
  queryOptions({
    queryKey: ["map-parser/grid-symbols", params],
    queryFn: ({ signal }): Promise<GetGridSymbolsReturnType> =>
      withWorker("map-parse", spawnWorker, terminate, (w) => w.getGridSymbols(params), { signal }),
    staleTime: Infinity,
  });

/** Direct (non-React) entry point; shares the React Query cache + dedup. */
export const getGridSymbolsAsync = (params: GetGridSymbolsParameters) =>
  queryClient.fetchQuery(getGridSymbolsQuery(params));
