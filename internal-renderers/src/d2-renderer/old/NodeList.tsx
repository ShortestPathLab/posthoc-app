...
export function NodeList({ canvas, nodes, hasCurrent = false }: NodeListProps) {
  if (!canvas || !nodes) {
...
export function LazyNodeList({ canvas }: LazyNodeListProps) {
  ...
  const [cachedNodes, dynamicNodes] = useMemo(() => {
    const cached = new Map<string | number, Event[]>();
    const dynamic = new Map<string | number, Event[]>();
    let i = 0;
    if (nodes) {
    ...
    <>
      {!!threshold && (
        <NodeList
          nodes={cachedNodes}
          canvas={canvas}
          hasCurrent={cachedNodes.size === step}
        />
      )}
